 ...
 *
 */
public class MiFrameOutputFormat<S extends Shape>
  extends FileOutputFormat<MiFrameKey, S> {
  
  private static final Log LOG = LogFactory.getLog(MiFrameOutputFormat.class);
  ...
  public static class IndexRecordWriter<S extends Shape> extends RecordWriter<MiFrameKey, S> {

    /**The partitioner used by the current job*/
    private MiFrameLocator frameLocator;
    ...
    private Map<Integer, MiFrame> frameMap = new ConcurrentHashMap<Integer, MiFrame>();
     ...
     */
    private Map<Integer, OutputStream> frameOutputs = new ConcurrentHashMap<Integer, OutputStream>();
    ...
    public IndexRecordWriter(TaskAttemptContext task, String name, Path outPath,
      ...
      this.frameLocator = MiFrameLocator.getLocator(conf);
    ...
    public IndexRecordWriter(MiFrameLocator partitioner, boolean replicate,
        String sindex, Path outPath, Configuration conf)
            throws IOException, InterruptedException {
      this.replicated = replicate;
      this.outFS = outPath.getFileSystem(conf);
      this.outPath = outPath;
      this.frameLocator = partitioner;
    ...
    @Override
    public void write( MiFrameKey mifKey, S value) throws IOException {
      int frameId = mifKey.getFrameId();
      int indexId = mifKey.getIndexId();
      
      if (frameId < 0) {
        // An indicator to close a partition
        int partitionToClose = -frameId - 1;
        this.closePartition(partitionToClose);
      } else {
        // An actual object that we need to write
        OutputStream output = getOrCreateDataOutput(frameId);
        tempText.clear();
        value.toText(tempText);
        byte[] bytes = tempText.getBytes();
        output.write( frameId );
        output.write( indexId );
        output.write(bytes, 0, tempText.getLength());
        output.write(NEW_LINE);
        MiFrame frame = frameMap.get( frameId );
        MicroIndex index = frame.getIndex(indexId );
        
        index.recordCount++;
        index.size += tempText.getLength() + NEW_LINE.length;
        index.expand(value);
     ...
     */
    private void closePartition(final int frameId) {
      
      final MiFrame frameInfo = frameMap.get(frameId);
      final OutputStream outStream = frameOutputs.get(frameId);
      final File tempFile = tempFiles.get(frameId);
        ...
        public void run() {
          try {
            ...
            if (localIndexer != null) {
              ...
              try {
                Path indexedFilePath = getFrameFile(frameId);
                frameInfo.filename = indexedFilePath.getName();
            ...
            if (replicated) {
              ...
              MicroIndex[] indexes = frameInfo.getIndexes();
              for( MicroIndex index : indexes )
              {
                CellInfo cellInfo = frameLocator.getPartition( index.getIndexId() );
                index.set( index.getIntersection( cellInfo));
              }
            }
            
            Text frameText = frameInfo.toText(new Text());
            synchronized (masterFile) {
              // Write partition information to the master file
              masterFile.write(frameText.getBytes(), 0, frameText.getLength());
            ...
            e.printStackTrace();
            throw new RuntimeException("Error closing partition: "+frameInfo, e);
          }
        }
      };
      ...
      frameOutputs.remove(frameId);
      tempFiles.remove(frameId);
     ...
     * is returned.
     * 
     * @param frameId - the ID of the partition
     * @return
     * @throws IOException 
     */
    private OutputStream getOrCreateDataOutput(int frameId) throws IOException {
      OutputStream out = frameOutputs.get(frameId);
      if (out == null) {
        ...
        if (localIndexer == null) {
          ...
          Path path = getFrameFile(frameId);
          out = outFS.create(path);
          frame.setFilename( path.getName() );
          
        } else {
          // Write to a temporary file that will later get indexed
          File tempFile = File.createTempFile(String.format("part-%05d", frameId), "lindex");
          out = new BufferedOutputStream(new FileOutputStream(tempFile));
          tempFiles.put(frameId, tempFile);
        }
     ...
     * @throws IOException 
     */
    private Path getFrameFile(int id) throws IOException {
    ...
    public void close(TaskAttemptContext task) throws IOException {
      try {
        ...
        for (Integer id : frameMap.keySet()) {
          closePartition(id);
          if (task != null)
            task.progress();
        }
  ...
  @Override
  public RecordWriter<MiFrameKey, S> getRecordWriter(TaskAttemptContext task)
      throws IOException, InterruptedException {
