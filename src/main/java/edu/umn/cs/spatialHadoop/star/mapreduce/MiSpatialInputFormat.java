...
*
*************************************************************************/
package edu.umn.cs.spatialHadoop.star.mapreduce;
 ...
 */
public class MiSpatialInputFormat<K extends Rectangle, V extends Shape>
    extends FileInputFormat<K, Iterable<V>> {
  
  private static final Log LOG = LogFactory.getLog(MiSpatialInputFormat.class);
  ...
  public RecordReader<K, Iterable<V>> createRecordReader(InputSplit split,
      ...
      if (extension.contains("mi_"))
      {
        return (RecordReader)new MiFrameRecordReader<V>();
      }
