...
import edu.umn.cs.spatialHadoop.star.indexing.MiFrameOutputFormat;
import edu.umn.cs.spatialHadoop.star.indexing.MiFrameOutputFormat.IndexRecordWriter;
 ...
 *
 */
public class MiFrameIndexer {
  private static final Log LOG = LogFactory.getLog(MiFrameIndexer.class);
  ...
  private static Job indexMapReduce(Path inPath, Path outPath,
    ...
    job.setJarByClass(MiFrameIndexer.class);
    ...
    //
    MiFrameLocator locator = createPartitionerAndLocator(inPath, outPath, conf, index);
    MiFrameLocator.setLocator(conf, locator );    
    ...
    job.setOutputFormatClass( MiFrameOutputFormat.class);
    MiFrameOutputFormat.setOutputPath(job, outPath);
    // Set number of reduce tasks according to cluster status
    ClusterStatus clusterStatus = new JobClient(new JobConf()).getClusterStatus();
    job.setNumReduceTasks(Math.max(1, Math.min( locator.getPartitionCount(),
  ...
  public static MiFrameLocator createPartitionerAndLocator(Path in, Path out,
      Configuration job, String partitionerName) throws IOException {
    return createPartitionerAndLocator(new Path[] {in}, out, job, partitionerName);
  }
   ...
   * @throws IOException
   */
  public static MiFrameLocator createPartitionerAndLocator(Path[] ins, Path out,
      Configuration job, String partitionerName) throws IOException {
    try {
      ...
      MiFrameLocator locator = new MiFrameLocatorRoundRobin( partitioner, numPartitions );
  ...
  private static void indexLocal(Path inPath, final Path outPath,
    ...
    final MiFrameLocator locator = createPartitionerAndLocator(inPath, outPath, conf, sindex);

    final IndexRecordWriter<Shape> recordWriter = new IndexRecordWriter<Shape>(
        locator, replicate, sindex, outPath, conf);
      ...
      while (reader.nextKeyValue()) {
        ...
        if (replicate) {
          for (final Shape s : shapes) {
            locator.overlapPartitions(s, new ResultCollector<MiFrameKey>() {
              @Override
              public void collect(MiFrameKey frameIndexId) {
                
                try {
                  recordWriter.write( frameIndexId, s);
                } catch (IOException e) {
          ...
          for (final Shape s : shapes) {
            MiFrameKey key = locator.overlapPartitions(s);
            if ( key != null ) {

              recordWriter.write( key, s);
            }
          }
        }
      }
    ...
    MiFrame tempPartition = new MiFrame();
